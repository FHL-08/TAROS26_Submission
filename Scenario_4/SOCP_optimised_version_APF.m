...
p_obs_3 = [1.2; 1.4];
p_obs_4 = [-1; 0];
obstacle_array = [p_obs_1 p_obs_2 p_obs_3 p_obs_4];
...
%%% memory allocation
consensus_opt_output = repmat([repmat(zeros(dim_state-1, 1), N, 1); ...
                               zeros(2*N, 1); 0], 1, mrs_size);
...
%%% Define YALMIP MOSEK options
options = sdpsettings('solver', 'mosek', 'verbose', 0);
options.mosek.MSK_IPAR_INTPNT_BASIS = 'MSK_ON';  % allows mosek to warm-start

%%% YALMIP ECOS options
% options = sdpsettings('solver', 'ecos');
% options.ecos.abstol = 1e-8; % Set the absolute feasibility tolerance
% options.ecos.reltol = 1e-8; % Set the relative feasibility tolerance
% options.ecos.maxit = 200; % Set the maximum number of iterations
% options.ecos.verbose = 0; % Turn off verbose output from the solver

% optimisation parameters
consensus_opt_params = sdpvar(length(consensus_opt_output), 1);

% solver inputs
p_diffeomorphism_k_init = sdpvar(4, 1);
p_h_i_dis = sdpvar(2, 1);
p_dh_i_dis = sdpvar(2, 1);
p_ddh_i_dis = sdpvar(2, 1);
p_angle_init = sdpvar(1, 1);
p_w_const_var = sdpvar(1, 1);
p_c_var = sdpvar(1, 1);
p_lambda_var = sdpvar(1, 1);
...
h_vel = p_dh_i_dis;
angle_var = p_angle_init;
formation_sys_objective = 0;

Constraints = [];
R_angle_var = [cos(angle_var) -sin(angle_var);
               sin(angle_var)  cos(angle_var)];
H_theta = p_lambda_var*R_angle_var*[0 1; 1 0]*(R_angle_var');
f_theta = p_c_var*[m*l_R*l_T  (J_B + m*l_T^2); m*l_L*l_T -(J_B + m*l_T^2)]*(R_angle_var');
de_i_var = S_matrix*p_diffeomorphism_k_next;
d_E_theta = ((de_i_var - h_vel)')*H_theta*(de_i_var - h_vel)*[-1; 1];
...
for z=1:N
    ...
    if (z == 1)
        h_accel = p_ddh_i_dis;
        torque_var = d_E_theta + f_theta*(opt_var_z - h_accel);
        Constraints = [Constraints, torque_lb <= torque_var <= torque_ub];
    else
        h_accel  = zeros(2, 1);
        s_var = consensus_opt_params(N*(dim_state-1) + ((z-2)*2+1));
        t_var = consensus_opt_params(N*(dim_state-1) + ((z-1)*2));
        Constraints = [Constraints, ... 
                       -p_w_const_var <= t_var <= 0, ...
                       -p_w_const_var^2 <= s_var <= 0, ...
                       norm(de_i_var - h_vel) <= (t_var + p_w_const_var), ...
                       norm([2*(t_var + p_w_const_var); ...
                             (s_var + p_w_const_var^2) - 1]) <= (s_var + p_w_const_var^2) + 1, ...
                       norm(opt_var_z) <= (1/(p_c_var*sigma_val))*(max_torque ...
                                             - p_lambda_var*(s_var + p_w_const_var^2))];
        formation_sys_objective = formation_sys_objective ...
                                  + intermediate_penalty*(s_var^2) ...
                                  + intermediate_penalty*(t_var^2);
    ...
    if (z == N)
        ...
        s_var = consensus_opt_params(N*(dim_state-1) + (N*2 - 1));
        t_var = consensus_opt_params(N*(dim_state-1) + N*2);
        varepsilon_var = consensus_opt_params(end);

        formation_sys_objective = formation_sys_objective ...
                                  + (p_diffeomorphism_k_next')*P*p_diffeomorphism_k_next ...
                                  + (opt_var_z')*R*opt_var_z...
                                  + intermediate_penalty*(s_var^2) ...
                                  + intermediate_penalty*(t_var^2) ...
                                  + quadratic_slack_penalty*(varepsilon_var^2);
        ...
        Constraints = [Constraints, ... 
                       ...
                       norm([2*(t_var + p_w_const_var); ...
                             (s_var + p_w_const_var^2) - 1]) <= (s_var + p_w_const_var^2) + 1, ...
                       norm(terminal_control_law) <= (1/(p_c_var*sigma_val))*(max_torque ...
                                                       - p_lambda_var*(s_var + p_w_const_var^2))...
                                                       - varepsilon_var, ...
                       varepsilon_var <= 0];
    else
        formation_sys_objective = formation_sys_objective...
                                  + (p_diffeomorphism_k_next')*Q*p_diffeomorphism_k_next ...
                                  + (opt_var_z')*R*opt_var_z;
    end
end

% Build optimizer
input_parameters = {p_diffeomorphism_k_init, p_h_i_dis, p_dh_i_dis, ...
                    p_ddh_i_dis, p_angle_init, p_w_const_var, p_c_var, p_lambda_var};
SOCP_controller = optimizer(Constraints, formation_sys_objective, options, ...
                            input_parameters, ...
                            consensus_opt_params);
solver_times = zeros(mrs_size, length(k));

% Admissibility Constraints
z_val_max = max(1./(diag(g_dis)*l_T));
c_val_max = scaled_radius*z_val_max;
lambda_val_max = 0.5*J_B*c_val_max*z_val_max;
psi_val_max = (norm([b_val; l_T])/r)*z_val_max;
w_const_worst = min([(wheel_vel_ub(1)/psi_val_max); sqrt(max_torque/lambda_val_max)]);
...
for j=1:length(k)
    ...
    q_l_solution = q_solver('x0', q_l, 'p', dq_l);
    ...
    p_ref = leader_distribution + Delta_j;
    global_formation_errors = (p_j_array_database - p_ref)*modified_laplacian;
    ...
    for i=1:mrs_size
        ...
        diffeomorphism_k_val = full(eta_func(p_j_array_database, p_jdot_database,...
        ...
        z_val = 1/(g_i_dis_val*l_T);
        c_val = scaled_radius*z_val;
        lambda_val = 0.5*J_B*c_val*z_val;
        psi_val = (norm([b_val; l_T])/r)*z_val;
        w_const = min([(wheel_vel_ub(1)/psi_val); sqrt(max_torque/lambda_val)]);

        controller_inputs = {diffeomorphism_k_val, h_i_dis_val, dh_i_dis_val, ddh_i_dis_val, ...
                             angle_val, w_const, c_val, lambda_val};
        % Solve the SOCP
        [solution_output, problem_status,~,~,~,diagnostics] = SOCP_controller(controller_inputs);

        if problem_status ~= 0
            disp(yalmiperror(problem_status));
            error('YALMIP failed at iteration j=%d for robot i=%d', j, i);
        end
        
        consensus_opt_output(:, i)= solution_output;
        optimal_Xi = consensus_opt_output(1:N*(dim_state-1), i);
...
rectangle('Position', [p_obs_3(1)-obstacle_radius,...
                       ...
                       2*obstacle_radius, 2*obstacle_radius],...
                      'Curvature', [1, 1], 'FaceColor',[0 0 0]);
rectangle('Position', [p_obs_4(1)-obstacle_radius,...
                       p_obs_4(2)-obstacle_radius,...
                       2*obstacle_radius, 2*obstacle_radius],...
                      'Curvature', [1, 1], 'FaceColor',[0 0 0]);
