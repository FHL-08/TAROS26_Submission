...
x_state_var_init = MX.sym('x_state_var_init', 5);
...
x_state_var_next = x_state_var_init;
...
for z=1:N
    ...
    % Runge-kutta 4th order discretisation
    k1 = dx_state_func(x_state_var_next, opt_var_z);
    x2 = x_state_var_next + 0.5*dt*k1;
    k2 = dx_state_func(x2, opt_var_z);
    x3 = x_state_var_next + 0.5*dt*k2;
    k3 = dx_state_func(x3, opt_var_z);
    x4 = x_state_var_next + 0.5*dt*k3;
    k4 = dx_state_func(x4, opt_var_z);

    torque_var = opt_var_z;
    wheel_bound_var = x_state_var_next(end-1:end);
    ...
    x_state_var_next = x_state_var_next + (dt/6)*(k1 + 2*k2 + 2*k3 + k4);
    
    h_i_vec = [h_i_dis_next; p_dh_i_dis];
    cart_pos = x_state_var_next(1:2);
    dcart_pos = k1(1:2);
    eta_k_var = h_i_vec + p_g_i_dis*[cart_pos; dcart_pos];
    h_i_dis_next = h_i_dis_next + p_dh_i_dis*dt;

    if (z == N)
        consensus_stage_cost = consensus_stage_cost + (eta_k_var')*P*eta_k_var;

        terminal_control_law = -K*eta_k_var;
        k1 = dx_state_func(x_state_var_next, terminal_control_law);
        x2 = x_state_var_next + 0.5*dt*k1;
        k2 = dx_state_func(x2, terminal_control_law);
        x3 = x_state_var_next + 0.5*dt*k2;
        k3 = dx_state_func(x3, terminal_control_law);
        x4 = x_state_var_next + 0.5*dt*k3;
        k4 = dx_state_func(x4, terminal_control_law);
        x_state_var_next = x_state_var_next + (dt/6)*(k1 + 2*k2 + 2*k3 + k4);
    
        torque_var = terminal_control_law;
        wheel_bound_var = x_state_var_next(end-1:end);
        torque_constraint_vec(N*(dim_state-1)+1:(N+1)*(dim_state-1)) = torque_var;
        wheel_vel_constraint_vec(N*(dim_state-1)+1:(N+1)*(dim_state-1)) = wheel_bound_var;
    else
        consensus_stage_cost = consensus_stage_cost + (eta_k_var')*Q*eta_k_var;
    end
end
...
all_params_sym = [x_state_var_init; p_h_i_dis; p_dh_i_dis; p_g_i_dis];
...
for j=1:length(k)
    ...
    for i=1:mrs_size
        ...
        numerical_param_values = [x_state_k; h_i_dis_val; dh_i_dis_val; g_i_dis_val];
        ...
        optimal_Tau = full(consensus_opt_output(:, i));
        ...
        next_optimal_Tau = optimal_Tau(dim_tau+1:end);
        consensus_opt_output(1:(N-1)*dim_tau, i) = next_optimal_Tau;
