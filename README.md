# Torque-Level Formation Control of Nonholonomic Wheeled Mobile Robots via Consensus-Based Feedback Linearised MPC

**Abstract:**     Real-time, torque-level formation control for nonholonomic wheeled mobile robots is fundamentally limited by the computational complexity of non-convex optimisation. This paper resolves this challenge with a distributed model predictive control framework that recasts the problem as a tractable Second-Order Cone Programme (SOCP). Our approach employs a two-stage input-output feedback linearisation to exactly transform the complex, underactuated dynamics into a Linear Time-Invariant subsystem, enabling convexification. We provide rigorous proofs of recursive feasibility and local asymptotic stability for the formation. Simulation results demonstrate that our SOCP controller retains the high performance and efficiency of non-convex feedback-linearised methods while achieving a nearly ninefold speed-up in computation time, positioning it as a promising direction for real-time deployment on resource-constrained platforms..
